...
By the time you reach the end, I hope you will look at this code and realize that almost every wise design decision follows from a single engineering constraint: inherited information is expensive. Once that constraint is accepted, many familiar AI architectures become surprisingly difficult to justify, while a population of continuously competing canonical modules begins to look almost inevitable, just like nature does.

This essay is therefore not a tutorial on Python. It is a tour through the engineering ideas hidden inside a few hundred lines of code that imitate how biological brains deploy their circuitry.
...
Think of it this way: if you had to build the city of Montreal, but all you had was a USB stick that holds 200 megabytes, you couldn't describe the city. You'd have to describe the machinery that grows the city. A set of rules that, when executed, produce the city through repetition.
...
## 3. Three Areas, Twenty-Five Copies Each? That Can't Be Right

Most programmers spend years trying to eliminate duplicated code.

So why does this program intentionally create three separate cortical areas, each with twenty-five nearly identical computational units?

At first this looks like poor software engineering.

In reality it is the central idea of the entire architecture.

Each unit within an area is almost identical. What distinguishes one from another is not its algorithm but its parameters. In the motor area, one unit slightly prefers food, another prefers home, another reacts more strongly to predators. In the navigation area, units respond to borders and explore. In the construction area, units respond to materials and the nest. Individually these differences are insignificant. Collectively they create populations capable of representing many competing behavioral tendencies without requiring seventy-five different algorithms.
...
Look at the code for one cortical area:
...
Four numbers. That's the entire description of one unit. The program then stamps out twenty-five copies for the motor area, each one slightly different because of the random initialization. Then it does the same for the navigation area with different parameters. Then again for the construction area.

The same mold produces seventy-five individuals, each with its own personality, organized into three functional areas.
...
Every microcircuit in every cortical area runs continuously.

Even those that are losing the competition continue to compute their activation. They are not disabled; they are merely less influential than the current winner. At every simulation step, seventy-five competing hypotheses about the creature's next movement coexist across three areas. The winners change as the sensory inputs change, but the competition itself never stops.
...
Each of the twenty-five copies in each area has an activity level. At every frame, that activity decays slightly. Then, the copies that best match the current sensory input receive a boost.

The boost is not binary. It's proportional to how well the copy's tuning matches the direction of whatever is being sensed. In the motor area, food, home, and predators each push activity in slightly different directions. In the navigation area, borders push activity away from walls. In the construction area, materials and the nest push activity toward building.

The result is a race that never finishes. Seventy-five runners across three tracks, each slowing down a little every step, and every step some of them get a push from the environment. The ones that get pushed hardest win their area this frame. Then decay starts again, and the races continue.
...
This is population coding. The direction the creature moves is not determined by any single copy. It is the vector average of all active copies across all areas, weighted by their current activity. The creature moves in the direction that emerges from the collective competition across all three cortical areas.
...
## 6. Growing Up: The Third Area

Here is another strange thing about the code.

The creature in its early moments is not able to build a nest. That capability doesn't show on day one. And when the time comes,
the program doesn't modify the existing circuits to add the new capability. It activates a third cortical area, with full inherited nest build capacities,
that has been sitting silent since birth.
...
The third area — the construction area, labeled N+2 in the code — has been present but silent, without permission to act. When it wakes, it doesn't overwrite anything. It blends in, with a share of the vote that grows slowly over time.

```
primary_motor = (1 - weight_n2) * old_motor + weight_n2 * new_motor
```

The old motor and navigation areas are never touched. Not one of their numbers changes. The new area simply starts contributing a small, growing fraction of the final decision.
...
The standard fix is to protect the old weights while still touching them. The other fix — the one this program uses — is to freeze the old networks completely and add a new area beside them. Zero risk of forgetting, because there's nothing shared left to overwrite.

There's a deeper reason this matters too. If the new area had to learn nest-building the moment it woke up, that would be expensive. Learning takes time and energy. The far cheaper move is for the new area to wake up already shaped — not an empty room waiting to be furnished, but a pre-built module wheeled into place and plugged into wiring that's already there.

The competence was paid for somewhere else. The creature just switches it on.

In biology, this mirrors myelination — the process that lets neural circuits fire efficiently continues well into adulthood, arriving late and gradually rather than all at once. New capabilities appear not by rewriting existing circuits but by adding new circuits alongside them.
...
This is not just an aesthetic choice. It prevents the creature from lurching between behaviors. It allows smooth transitions, smooth responses, smooth decisions. In biological terms, it replaces hard switches with continuous, gradual gates — exactly what evolution favors when abrupt changes would be costly or dangerous.

---

## 8. The Final Surprise: There Is No Memory

By this point many readers have searched the code looking for a map.

Or a list of visited locations.

Or perhaps a planner.

You won't find one.

That absence is deliberate.

The creature does not survive because it remembers the world.

It survives because its continuously competing populations across three cortical areas react appropriately to the present.
...
Whether that answer ultimately extends to biological intelligence remains an open scientific question.

For this small creature, however, the experiment is surprisingly successful.

The creature forages for food. It flees from the wolf. It builds a nest. It does all of this without a map, without a planner, without a decision tree, without a state machine.

What it has instead is three cortical areas, each with twenty-five copies of a four-number mold, arranged in rings, competing continuously, with the third area waking up when the time is right.

---

## 9. So Why Write Code Like This?
...
If you tried to write an `if` statement for every situation the creature might encounter, you would never finish. If you tried to train a neural network on every possible scenario, you would never have enough data. If you tried to build a map of the world, the creature would be helpless in any environment it hadn't memorized.

This architecture takes a different approach. It builds a small, repeatable unit and deploys it many times across multiple functional areas. The unit is simple. The deployment is systematic. The behavior emerges from the interaction.
...
*A tour through the engineering ideas hidden inside a few hundred lines of Python that imitate biological brains deployment — inspired by the biological work of Zador (genomic bottleneck), Mountcastle (cortical columns), Raichle (default mode network), Kirschner and Gerhart (facilitated variation), and the engineering work of Holland and Goldberg (genetic algorithms), Rusu et al. (Progressive Neural Networks), Kirkpatrick et al. (Elastic Weight Consolidation), and Chang (Evolving Cooperative Neural Agents).*
